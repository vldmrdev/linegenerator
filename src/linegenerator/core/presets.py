...
class LogPreset(StrEnum):
    ...
    FASTAPI = 'INFO:     {ipv4_public}:{port} - "{method} {path} HTTP/1.1" {status} {status_text}'
    # POSTGRES = "{timestamp_postgres} [{pid}]: [1-1] user={user},db={db},app={app},client={client} LOG:  duration: {duration} ms  statement: {statement}"
